...
export function ProductGallery({ media, name }: { media: string[]; name: string }) {
  ...
  return <section className="product-gallery" aria-label={`Galeria de fotos e vídeos de ${name}`} onKeyDown={onKeyDown} tabIndex={0} onTouchStart={(event) => { touchStart.current = event.changedTouches[0]?.clientX ?? null; }} onTouchEnd={(event) => { if (touchStart.current === null) return; const distance = event.changedTouches[0].clientX - touchStart.current; if (Math.abs(distance) > 45) { if (distance > 0) previous(); else next(); } touchStart.current = null; }}>
