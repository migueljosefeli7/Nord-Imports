...
export function ProductCard({ product, index = 0, featured = false }: { product: Product; index?: number; featured?: boolean }) {
  return <article className={`product-card ${featured ? "featured" : ""}`}>
    <Link href={`/produto/${product.slug}`} className="product-image" aria-label={`Ver ${product.name}`}>
      <Image src={product.image} alt={product.name} fill sizes={featured ? "(max-width: 768px) 100vw, 50vw" : "(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"} />
    ...
    <div className="product-info"><div><p>{product.brand} · {product.subcategory}</p><h3><Link href={`/produto/${product.slug}`}>{product.name}</Link></h3></div><span>Sob consulta</span></div>
  </article>;
}
