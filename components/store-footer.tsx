...
export function StoreFooter() {
  ...
  useEffect(() => { fetch("/api/catalog").then((response) => response.ok ? response.json() : null).then((data) => { if (data) { setBrandList(data.brands); setWhatsapp(data.settings.whatsapp); } }).catch(() => undefined); }, []);
